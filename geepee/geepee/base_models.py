...
from greedy import get_params_SGP
from greedy import get_params_SGP_old
...
class Base_Model(object):
    ...
    def init_hypers(self, y_train, x_train=None, init_type='default'):
        """Summary
    ...
    def optimise(
        ...
        if str(reinit_hypers) == 'True':
            print 'Default initialisation'
            init_params_dict = self.init_hypers(self.y_train)

        elif  reinit_hypers == 'greedy':
            init_params_dict = self.init_hypers(self.y_train)
            print 'SGP greedy initialisation'
            params_sgp = get_params_SGP(self.x_train, self.y_train, self.Ms[0])
            # Make changes in input layer
            # key_list = list(set(params_sgp.keys()) - set(['eta2_0','eta1_R_0']))
            # for i in key_list:
            for i in params_sgp.keys():
                print i
                init_params_dict[i] = params_sgp[i]
            # init_params_dict['eta2_0'] = np.concatenate((params_sgp['eta2_0'],params_sgp['eta2_0']),
            #                                 axis=0)
        ...
        elif reinit_hypers == 'long_short':
            print 'Long short initialisation'
            init_params_dict = self.init_hypers(self.y_train, init_type=reinit_hypers)
        elif reinit_hypers == 'fixed_zu':
            print 'Fixed zu_0 initialisation'
            init_params_dict = self.init_hypers(self.y_train, init_type=reinit_hypers)

        elif str(reinit_hypers) == 'False': 
...
class Base_SGP_Layer(object):
    ...
    def init_hypers(self, x_train=None, key_suffix='', init_type='default'):
        """Summary
        ...
        def init_default(x_train=None, key_suffix=''): 
            N = self.N
            M = self.M
            Din = self.Din
            Dout = self.Dout
        ...
        def init_short_long(x_train=None, key_suffix=''): 
            N = self.N
            M = self.M
            Din = self.Din
            Dout = self.Dout
            ...
            if x_train is None:
                ls = np.log(2 * np.ones((Din, )) + 0.1 * np.random.rand(Din, ))
                sf = np.log(np.array([1]))
                zu = np.tile(np.linspace(-1, 1, M).reshape((M, 1)), (1, Din))
            else:
                if N < 10000:
        ...
        def init_long_short(x_train=None, key_suffix=''): 
            N = self.N
            M = self.M
            Din = self.Din
            Dout = self.Dout
            ...
            if x_train is None:
                ls = np.log(0.05 * np.ones((Din, )) + 0.1 * np.random.rand(Din, ))
                sf = np.log(np.array([1]))
                zu = np.tile(np.linspace(-1, 1, M).reshape((M, 1)), (1, Din))
            else:
                if N < 10000:
        ...
        def init_fixed_zu(x_train=None, key_suffix=''): 
            N = self.N
            M = self.M
            Din = self.Din
            Dout = self.Dout
            ...
            if x_train is None:
                ...
                if N < 10000:
                    ...
                    centroids = np.linspace(x_min, x_max, M).reshape(-1,1)
                else:
                    randind = np.random.permutation(N)
                    centroids = x_train[randind[0:M], :]
                zu = centroids
        ...
        if init_type == 'default':
            params = init_default(x_train, key_suffix)
        elif init_type == 'short_long': 
            params =  init_short_long(x_train, key_suffix)
        elif init_type == 'long_short': 
            params =  init_long_short(x_train, key_suffix)
        elif init_type == 'fixed_zu':
            params = init_fixed_zu(x_train, key_suffix)
...
class Base_SGPLVM(Base_Model):
    ...
    def init_hypers(self, y_train, init_type='default'):
        """Summary
...
class Base_SGPR(Base_Model):
    ...
    def init_hypers(self, y_train, init_type='default'):
        ...
        Returns:
            TYPE: Description
        """
        sgp_params = self.sgp_layer.init_hypers(self.x_train, init_type=init_type)
        lik_params = self.lik_layer.init_hypers()
        init_params = dict(sgp_params)
        init_params.update(lik_params)
        return init_params
